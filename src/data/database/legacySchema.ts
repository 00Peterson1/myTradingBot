...
export function initializeLegacySchema(db: Database.Database): void {
  ...
  for (const [col, def] of newSymbolCols) {
    const columns = db.pragma('table_info(symbols)') as { name: string }[];
    if (!columns.some(column => column.name === col)) db.exec(`ALTER TABLE symbols ADD COLUMN ${col} ${def}`);
  }
  ...
  for (const [col, def] of newProfileCols) {
    const columns = db.pragma('table_info(market_profiles)') as { name: string }[];
    if (!columns.some(column => column.name === col)) db.exec(`ALTER TABLE market_profiles ADD COLUMN ${col} ${def}`);
  }
}
