...
export function cointegrationTest(
  ...
  let sumB2 = 0;
  for (let i = 0; i < n; i++) {
    sumAB += logPricesA[i]! * logPricesB[i]!;
    sumB2 += logPricesB[i]! * logPricesB[i]!;
  }
  ...
  let resSum = 0;
  for (let i = 0; i < n; i++) {
    residuals[i] = logPricesA[i]! - alpha - beta * logPricesB[i]!;
    resSum += residuals[i]!;
  }
  ...
  let sqDiffSum = 0;
  for (let i = 0; i < n; i++) {
    const diff = residuals[i]! - spreadMean;
    sqDiffSum += diff * diff;
  }
  ...
  let sumLagged2 = 0;
  for (let i = 0; i < n - 1; i++) {
    sumLaggedDelta += lagged[i]! * delta[i]!;
    sumLagged2 += lagged[i]! * lagged[i]!;
  }
  ...
  let errSqSum = 0;
  for (let i = 0; i < n - 1; i++) {
    const err = delta[i]! - gamma * lagged[i]!;
    errSqSum += err * err;
  }
